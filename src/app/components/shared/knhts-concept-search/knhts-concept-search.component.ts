...
export class KnhtsConceptSearchComponent implements OnInit {
  ...
  @Input() required = true;
  /** Optional override search function — when provided it is called instead of KnhtsService.searchConcepts */
  @Input() searchFn?: (
    searchTerm: string,
  ) => Observable<KnhtsConceptSearchResult[]>;
  ...
  ngOnInit(): void {
    if (this.initialConcept) {
      ...
      this.searchControl.setValue(
        this.initialConcept as KnhtsConceptSearchResult,
        { emitEvent: false },
      );
    }
    ...
    this.filteredOptions$ = this.searchControl.valueChanges.pipe(
      ...
      switchMap((value) => {
        const search$ = this.searchFn
          ? this.searchFn(value)
          : this.knhtsService.searchConcepts(value);
        return search$.pipe(
          catchError(() => of([])),
